...
DEVELOPMENT = ROOT / 'artifacts/captcha-additions-20260909'
RESERVED = ROOT / 'artifacts/captcha-training-20260908'
OUTPUT = ROOT / 'artifacts/captcha-validation-20260909'
...
def frozen_files():
    files = list((ROOT / 'EpicRPGBot.UI/Captcha').rglob('*.cs'))
    files += list((ROOT / 'Items').rglob('*.webp'))
    files += [ROOT / 'items.json', ROOT / 'tools/captcha/fine-policy.json']
