...
DIRECTORY = ROOT / 'artifacts/captcha-validation-20260909'
...
def verify_frozen_inputs(freeze):
    ...
    development = ROOT / 'artifacts/captcha-additions-20260909/v9-development-results.json'
...
def main():
    ...
    (ROOT / 'captcha-local.json').write_bytes((DIRECTORY / 'policy.json').read_bytes())
    source_summary_path = ROOT / 'artifacts/captcha-training-20260908/summary.json'
    source_summary = read(source_summary_path)
    source_summary.update(holdoutEvaluated=True,
                          holdoutEvaluation='template-fine-v9; 100/100; artifacts/captcha-validation-20260909')
    ...
    result = dict(completedUtc=datetime.now(timezone.utc).isoformat(), fingerprint=freeze['fingerprint'],
                  ...
                  minimumAcceptedMargin=min(prediction['Candidates'][0]['Score'] - prediction['Candidates'][1]['Score']
                                            for prediction in predictions), productionPolicy='captcha-local.json',
                  policySha256=digest(ROOT / 'captcha-local.json'))
