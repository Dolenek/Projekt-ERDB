...
def main():
    ...
    records=json.loads((ROOT/"artifacts/puzzle-dataset/calibration.json").read_text());results=[]
    start=time.time()
    for record in records:
        original=cv2.imread(str(ROOT/"artifacts/puzzle-dataset"/record["image"]))
    ...
    (ROOT/"artifacts/puzzle-dataset/normalized-prototype.json").write_text("[\n"+",\n".join(json.dumps(r,separators=(",",":")) for r in results)+"\n]\n")
