...
def main():
    choices=variants();records=json.loads((ROOT/"artifacts/puzzle-dataset/calibration.json").read_text())
    started=time.time()
    def evaluate(record):
        ranking=classify(ROOT/"artifacts/puzzle-dataset"/record["image"],choices)
    ...
    (ROOT/"artifacts/puzzle-dataset/calibration-prototype.json").write_text("[\n"+",\n".join(json.dumps(r,separators=(",",":")) for r in results)+"\n]\n")
