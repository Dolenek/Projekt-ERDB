...
public sealed class AdaptivePuzzleTests
{
    [WindowsFact]
    public void QuestionLocator_HandlesHorizontalOffsetsAndOmitsText()
    {
        foreach (var textLeft in new[] { 75, 115, 180 })
        {
            ...
            var bounds = new PuzzleQuestionRegionLocator().Locate(card);
    ...
    public void QuestionLocator_RejectsCardWithoutQuestion()
    {
        ...
        Assert.Throws<ArgumentException>(() => new PuzzleQuestionRegionLocator().Locate(card));
    }
    ...
    public void LineFilter_RemovesThinInterferenceAndPreservesBroadItem()
    {
        ...
        using var cleaned = new PuzzleColoredLineFilter().Apply(card);
    ...
    public async Task AdaptivePipeline_RecognizesKeyButDoesNotReuseLegacySeal()
    {
        ...
        policy.Pipeline = LocalPuzzlePolicy.AdaptivePipeline;
    ...
    {
        var root = LocalPuzzleTransformTests.RepositoryRoot();
        var policy = LocalPuzzlePolicy.Load(Path.Combine(root, "tools/puzzle/adaptive-policy.json"));
        using var provider = new LocalPuzzleAnswerProvider(Path.Combine(root, "Items"),
            PuzzleItemCatalog.Load(Path.Combine(root, "items.json")), policy);
        var bytes = File.ReadAllBytes(Path.Combine(root,
            "artifacts/puzzle-training-20260908/images/1196874297508106250.png"));
        var result = await provider.SolveAsync(bytes, default);
