...
public sealed class LocalPuzzleImageTests
{
    [WindowsFact]
    ...
    {
        var root = RepositoryRoot();
        Assert.ThrowsAny<IOException>(() => new LocalPuzzleAnswerProvider(
            Path.Combine(root, "not-present"), PuzzleItemCatalog.Load(Path.Combine(root, "items.json")), new LocalPuzzlePolicy()));
    }
    ...
    private static LocalPuzzleAnswerProvider Create(string root) => new(
        Path.Combine(root, "Items"), PuzzleItemCatalog.Load(Path.Combine(root, "items.json")),
