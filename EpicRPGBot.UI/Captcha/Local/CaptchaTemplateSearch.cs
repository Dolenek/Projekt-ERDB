...
namespace EpicRPGBot.UI.Captcha.Local
{
    internal sealed class CaptchaTemplateSearch : IDisposable
    {
        ...
        private readonly Dictionary<string, CaptchaTemplatePose> _seeds = new Dictionary<string, CaptchaTemplatePose>(StringComparer.Ordinal);

        public CaptchaTemplateSearch(CaptchaScene scene, bool spectral = false)
        {
            _scene = scene;
            _maximumLuminance = spectral && CaptchaLuminance.IsGrayscale(scene.Colors, scene.Binary);
        }
        public IEnumerable<CaptchaCandidate> Candidates => _best.Values;
        public IEnumerable<CaptchaTemplatePose> Seeds => _seeds.Values;
        ...
        public void Evaluate(CaptchaTemplateVariant variant, bool retainSeed = true)
        {
            ...
            if (retainSeed) _seeds[variant.Label] = new CaptchaTemplatePose(variant);
        }
