...
{
    public sealed class LocalPuzzleAnswerProvider : IPuzzleAnswerProvider, IDisposable
    {
        private readonly ILocalPuzzleRecognizer _recognizer;
        ...
        public LocalPuzzleAnswerProvider(string templateDirectory, PuzzleItemCatalog catalog, LocalPuzzlePolicy policy)
            : this(new TemplatePuzzleRecognizer(templateDirectory, catalog,
        ...
        public LocalPuzzleAnswerProvider(ILocalPuzzleRecognizer recognizer, LocalPuzzlePolicy policy)
        {
        ...
        private PuzzleAnswerResult Solve(byte[] bytes, CancellationToken cancellationToken)
        {
            ...
            catch (ArgumentException ex) { return PuzzleAnswerResult.NoMatch("local", ex.Message); }
            catch (OpenCvSharp.OpenCVException ex) { return PuzzleAnswerResult.NoMatch("local", "Image processing failed: " + ex.Message); }
        }

        private PuzzleAnswerResult Decide(IReadOnlyList<PuzzleCandidate> ranking)
        {
            ...
            return PuzzleAnswerResult.Classified(ranking, accepted, AutomaticAnswersValidated,
