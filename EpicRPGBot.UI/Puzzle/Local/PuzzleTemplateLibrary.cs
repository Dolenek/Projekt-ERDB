...
{
    internal sealed class PuzzleTemplateLibrary : IDisposable
    {
        ...
        private readonly List<PuzzleTemplateVariant> _variants = new List<PuzzleTemplateVariant>();
        private readonly Dictionary<string, OpenCvSharp.Mat> _originals = new Dictionary<string, OpenCvSharp.Mat>();
        public IReadOnlyList<PuzzleTemplateVariant> Variants => _variants;
        public IReadOnlyList<string> Labels { get; private set; }
        public string Fingerprint { get; private set; }

        public static PuzzleTemplateLibrary Load(string directory, PuzzleItemCatalog catalog, bool includeKey = false,
            bool learned = false, bool croppedTraining = false)
        {
            ...
            if (labels.Any(label => !catalogLabels.Contains(label)) ||
                ...
                throw new InvalidDataException("Local puzzle catalog does not match the pipeline's supported items.");
            var library = new PuzzleTemplateLibrary { Labels = Array.AsReadOnly(labels) };
            try
            {
        ...
        {
            var original = PuzzleTemplateTransforms.ReadTemplate(Path.Combine(directory, label + ".webp"));
            ...
            foreach (var aspect in new[] { 0.65, 1.0, 1.5 })
                for (var angle = -30; angle <= 30; angle += 15)
                    for (var length = 12; length <= 56; length += 4)
                        using (var transformed = PuzzleTemplateTransforms.Transform(original, angle, length, aspect))
                            _variants.Add(new PuzzleTemplateVariant(label, transformed, angle, length, aspect, sourceKey));
        }
        ...
        private void AddLearned(string directory, string folder)
        {
            ...
            if (paths.Length == 0) throw new InvalidDataException("Missing learned puzzle templates.");
            using (var stream = new MemoryStream())
            {
                ...
                foreach (var path in paths)
                {
                    ...
                    AddOriginal(label, key, PuzzleTemplateTransforms.ReadTemplate(path));
                }
                using (var sha = SHA256.Create())
        ...
        public PuzzleTemplateVariant Refine(PuzzleTemplatePose seed, int angle, int length, double aspect)
        {
            using (var transformed = PuzzleTemplateTransforms.Transform(_originals[seed.SourceKey], angle, length, aspect))
                return new PuzzleTemplateVariant(seed.Label, transformed, angle, length, aspect, seed.SourceKey);
        }
