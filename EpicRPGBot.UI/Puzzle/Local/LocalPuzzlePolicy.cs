...
{
    public sealed class LocalPuzzlePolicy
    {
        ...
        public static LocalPuzzlePolicy Load(string path)
        {
            var policy = JsonSerializer.Deserialize<LocalPuzzlePolicy>(File.ReadAllText(path));
            if (policy == null || !IsSupportedPipeline(policy.Pipeline) ||
                !IsUnitValue(policy.MinimumScore) || !IsUnitValue(policy.MinimumMargin))
                throw new InvalidDataException("Invalid local puzzle policy.");
            return policy;
        }
