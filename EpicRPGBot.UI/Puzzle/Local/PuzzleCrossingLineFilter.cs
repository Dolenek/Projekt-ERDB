...
{
    internal sealed class PuzzleCrossingLineFilter : IPuzzleInterferenceFilter
    {
        private readonly double _thinRatio;
        public PuzzleCrossingLineFilter(double thinRatio = 3) { _thinRatio = thinRatio; }
        ...
        public Mat Apply(Mat colors)
        {
            using (var removal = new Mat(colors.Size(), MatType.CV_8UC1, Scalar.All(0)))
            {
                ...
                if (Cv2.CountNonZero(removal) == 0) return new PuzzleColoredLineFilter().Apply(colors);
                using (var kernel = Cv2.GetStructuringElement(MorphShapes.Rect, new Size(3, 3)))
                    Cv2.Dilate(removal, removal, kernel);
                using (var cleaned = new Mat())
                {
                    Cv2.Inpaint(colors, removal, cleaned, 3, InpaintTypes.Telea);
                    return new PuzzleColoredLineFilter().Apply(cleaned);
                }
            }
        }
