...
{
    public sealed class PuzzleSolverService : IDisposable
    {
        private readonly IPuzzleImageSource _images;
        private readonly Lazy<IPuzzleAnswerProvider> _provider;
        ...
        public PuzzleSolverService(IDiscordChatClient chatClient)
            : this(new PuzzleImageSource(chatClient ?? throw new ArgumentNullException(nameof(chatClient))),
                () => new PuzzleProviderFactory().Create(PuzzleSettings.LoadDefault()),
                () => PuzzleSettings.LoadDefault().AutomaticAnswersEnabled) { }

        public PuzzleSolverService(IPuzzleImageSource images, Func<IPuzzleAnswerProvider> provider,
            Func<bool> automaticAnswersEnabled)
        {
            _images = images ?? throw new ArgumentNullException(nameof(images));
            _provider = new Lazy<IPuzzleAnswerProvider>(provider);
        ...
        {
            if (Volatile.Read(ref _disposed) != 0) throw new ObjectDisposedException(nameof(PuzzleSolverService));
            ...
            using (var cancellation = new CancellationTokenSource())
            {
                ...
                try
                {
                    ...
                    if (bytes == null) { report?.Invoke("Puzzle image unavailable. Waiting for manual resolution."); return; }
                    await RecognizeAndSendAsync(bytes, sendAnswer, incidentIsCurrent, report, cancellation.Token);
                }
                catch (OperationCanceledException) { report?.Invoke("Puzzle attempt cancelled."); }
                catch (Exception ex) { report?.Invoke("Puzzle solver failed; waiting for manual resolution: " + ex.Message); }
                finally
                {
                    _attempt = null;
            ...
            Func<bool> incidentIsCurrent, Action<string> report, CancellationToken token)
        {
            ...
            report?.Invoke(sent ? "Puzzle answer '" + result.Label + "' sent. Waiting for EPIC GUARD confirmation."
                : "Puzzle answer not sent. Waiting for manual resolution.");
        }
