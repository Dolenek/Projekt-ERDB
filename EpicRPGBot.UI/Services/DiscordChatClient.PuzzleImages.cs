...
namespace EpicRPGBot.UI.Services
{
    public sealed partial class DiscordChatClient
    {
        ...
        public async Task<string> GetMessageImageUrlForMessageIdAsync(string messageId)
        {
            ...
            try
            {
                var result = await _web.CoreWebView2.ExecuteScriptAsync(BuildPuzzleImageCandidateScript(messageId));
                var payload = ParsePuzzleImageCandidatePayload(result);
        ...
        public async Task<byte[]> CaptureMessageImagePngAsync(string messageId)
        {
            ...
            try
            {
                var json = await _web.CoreWebView2.ExecuteScriptAsync(BuildPuzzleImageCandidateScript(messageId));
                var payload = ParsePuzzleImageCandidatePayload(json);
        ...
        private static string BuildPuzzleImageCandidateScript(string messageId)
        {
...
(() => {{
  ...
  return {{
    ...
    h:best.h
        ...
        private static PuzzleImageCandidatePayload ParsePuzzleImageCandidatePayload(string rawResult)
        {
            ...
            try
            {
                using (var document = JsonDocument.Parse(payload))
                {
                    ...
                    return new PuzzleImageCandidatePayload(
                        ReadBoolean(root, "ok"),
                        ReadString(root, "url"),
                        ReadDouble(root, "x"),
                        ReadDouble(root, "y"),
        ...
        private sealed class PuzzleImageCandidatePayload
        {
            public PuzzleImageCandidatePayload(bool ok, string url, double x, double y, double width, double height)
            {
                Ok = ok;
