...
5. Captcha sending uses the shared send lane, checks the current incident and
...
- `CAPTCHA_ITEM_NAMES_FILE`: default `items.json`.
- `CAPTCHA_TEMPLATES_DIR`: default `Items`.
- `CAPTCHA_LOCAL_POLICY_FILE`: default `captcha-local.json`.
- `CAPTCHA_AUTO_SEND`: default `1`; still subject to the validation gate.
- `CAPTCHA_DEBUG_CAPTURE`: default `0`; set `1` to save attachment bytes.
- `CAPTCHA_DEBUG_DIR`: default `artifacts/captcha-debug`.
- `CAPTCHA_SELFTEST=1` runs local replay on startup.
- `CAPTCHA_SELFTEST_REPLAY_DIR`: labeled attachments named `apple__example.png`
