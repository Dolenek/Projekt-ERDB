...
The EPIC GUARD flow tracks the Discord message id for each detected guard prompt.
...
- Typical lines include:
  ...
  - local recognition results and elapsed time,
  - chosen answer,
...
- A confident local catalog match is sent only when the provider is validated and automatic answers are enabled.
