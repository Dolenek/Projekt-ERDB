...
- If EPIC GUARD interrupts `rpg cd`, a tracked command, or `rpg use time cookie`, the command is not sent again. The engine uses a result attached to the guard-clear message or delivered as the next grouped EPIC RPG message, then the loop continues from that reply.
