# Legacy puzzle recognition evaluation

## Reproducible evidence

The legacy v1/v2 pipelines use the same 15 templates and
the same acceptance thresholds (score 0.65, margin 0.025).
`template-correlation-v1` has its separate policy in `tools/puzzle/legacy-policy.json`.
`template-refinement-v2` is an optional evaluation pipeline without a validation seal.
The runtime contract and configuration live in [puzzle-solver.md](puzzle-solver.md).
The shipped v9 results live in [trained evaluation](puzzle-trained-evaluation.md).
...
Evidence is in `artifacts/puzzle-dataset/baseline-calibration.json` and
...
independent evidence under [the validation procedure](puzzle-validation.md).
The legacy v1 policy retains its original limits and validation fingerprint.

See [the additional 500-image evaluation](puzzle-expansion-results.md) for current
