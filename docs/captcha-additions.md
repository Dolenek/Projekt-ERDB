...
`artifacts/captcha-additions-20260909/` contains 100 original attachments collected
...
existing captcha image directories and within the batch. The 94 retained images
...
Descriptions follow [the established annotation vocabulary](captcha-dataset-expansion.md).
...
`tools/captcha/prepare_development_audit.py` reproduces that correction and the
...
See [trained recognition](captcha-trained-recognition.md) and
[validation](captcha-validation.md).
