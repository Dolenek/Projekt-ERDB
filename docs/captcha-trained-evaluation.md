# Trained captcha evaluation

## Shipped pipeline

`captcha-local.json` selects `template-fine-v9` with score 0.65 and margin 0.025.
...
Algorithm details live in [trained recognition](captcha-trained-recognition.md).
...
The reserved split comes from [the sixteen-class batch](captcha-training-dataset.md).
...
`artifacts/captcha-validation-20260909/` contains:
...
`artifacts/captcha-additions-20260909/v9-development-results.json` and its eight
...
Build the replay tool, then use the [replay CLI](captcha-validation.md):

```powershell
tools/CaptchaReplay/bin/Release/net48/CaptchaReplay.exe . artifacts/captcha-validation-20260909/holdout.json artifacts/captcha-validation-20260909/replay-check.json
```
...
[the runtime contract](captcha-solver.md). Further tuning needs a fresh holdout.
