# Trained local captcha recognition
...
The validated production policy is `captcha-local.json`; the unsealed evaluation
policy is `tools/captcha/fine-policy.json`. Acceptance thresholds remain score
...
`tools/captcha/train_gray_templates.py` reproduces the assets without opening a
...
`CaptchaCrossingLineFilter` implements `ICaptchaInterferenceFilter`. It finds
...
`CaptchaLuminance` detects low-chroma scenes using mean foreground chroma below
...
`CaptchaTemplatePose` retains transform metadata without keeping disposed OpenCV
...
`tools/captcha/run_partitioned_replay.py` partitions development manifests across
...
See [new attachment provenance](captcha-additions.md),
[evaluation results](captcha-trained-evaluation.md), and
[validation requirements](captcha-validation.md).
