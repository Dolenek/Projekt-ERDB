...
5. Warm all three Discord tabs once during startup so each WebView2 surface is realized before the user switches tabs.
6. Initialize all three WebView2 tabs with a shared persistent profile under `%LocalAppData%/EpicRPGBot.UI/WebView2`.
7. Navigate the bot and player tabs to the saved bot channel URL, or `https://discord.com/channels/@me` if the URL is empty and fallback is enabled.
...
- `Stop Bot` stops engine timers but keeps all three tabs open.
