# Additional captcha attachments

## Source and scope

`artifacts/captcha-dataset-expansion-20260908/` contains 500 original PNG attachments
...
- `holdout.json`: 100 supported examples reserved before tuning and evaluated
  by both fixed pipelines; see [the results](captcha-expansion-results.md).
...
`key` is outside the legacy v1/v2 recognition sets. The [clean/adaptive pipelines](captcha-adaptive.md)
...
V1 rejects all five; v2 falsely accepts one. See [the results](captcha-expansion-results.md).
...
`tools/captcha/collect_dataset.py` accepts ephemeral MCP-observed URL records,
...
`tools/captcha/finalize_dataset.py <batch-directory>` combines acquisition metadata
...
See [validation requirements](captcha-validation.md) and
[the local runtime contract](captcha-solver.md).
