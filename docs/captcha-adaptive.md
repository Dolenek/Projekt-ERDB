# Adaptive local captcha recognition
...
API. Their evaluation policies are `tools/captcha/clean-policy.json` and
`tools/captcha/adaptive-policy.json`. Legacy v1/v2 still recognize their original
15 labels; adding the key catalog entry does not change their fingerprints.

`TemplateCaptchaRecognizer` selects preprocessing and label set by pipeline
identity. `ILocalCaptchaRecognizer` remains the provider extension point.
...
`CaptchaQuestionRegionLocator` owns text-region detection.
`CaptchaCardPreprocessor` owns cropping and downsampling.
`ICaptchaInterferenceFilter` permits alternative local interference processing.
`CaptchaColoredLineFilter` groups saturated pixels by quantized BGR color and
...
`artifacts/captcha-dataset-expansion-20260908/images/1542152978981458002.png`.
...
See [the new dataset](captcha-training-dataset.md) for provenance, split boundaries
and evaluation status, and [the validation contract](captcha-validation.md).
