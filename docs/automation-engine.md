...
Engine lifecycle:
- `Start()` uses the saved work command for the current area, starts message polling, and waits for the startup `rpg cd` snapshot before scheduling daily/weekly/card-hand/hunt/adventure/training/work/farm/lootbox.
- `Stop()` stops all tracked-command and message timers, including card hand, and marks the engine as stopped.
...
Timer behavior:
- Tracked-command timers are one-shot timers that are re-armed after the EPIC RPG reply is seen.
...
- Card hand uses `rpg card hand` and runs the complete interactive game while holding the global send lane; see [Card hand automation](card-hand-automation.md).
...
- The right-side cooldown panel starts all automated tracked cooldowns when the EPIC RPG confirmation reply is received, without waiting for the next `rpg cd`; card hand uses a 24-hour fallback.
...
- When the UI parses a fresh `rpg cd` snapshot or a time-cookie reduction, the engine resyncs all automated tracked timers, including card hand, from the tracked cooldown panel and clears stale pending replies.
