# Puzzle evaluation on the additional dataset

## Scope

The legacy v1/v2 recognizers were evaluated on all 500 attachments in
`artifacts/puzzle-dataset-expansion-20260908/`, using unchanged templates,
...
Use [the replay CLI](puzzle-validation.md) for development and holdout manifests.
Select v1 with `--policy tools/puzzle/legacy-policy.json` or v2 with
`--policy tools/puzzle/refined-policy.json`.
For unsupported cards also supply `--allow-unsupported`, never `--validate`.
Run `python tools/puzzle/summarize_replay.py <batch-directory>` to audit report
...
See [dataset provenance and annotation semantics](puzzle-dataset-expansion.md).
For the shipped 16-class pipeline, see [trained evaluation](puzzle-trained-evaluation.md).
