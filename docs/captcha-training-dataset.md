# Sixteen-class captcha dataset

## Acquisition and annotation

`artifacts/captcha-training-20260908/` contains 750 original attachments from
...
`tools/captcha/collect_webview_batch.py` attaches to that MCP-created WebView's
...
as [the earlier batch](captcha-dataset-expansion.md).
...
`tools/captcha/prepare_training_batch.py` creates a deterministic stratified split
...
[trained evaluation](captcha-trained-evaluation.md). The validation directory's
...
See [adaptive recognition](captcha-adaptive.md) and [replay usage](captcha-validation.md).
