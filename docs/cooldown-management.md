...
Runtime scheduling:
- The bot still arms hunt/adventure/work/farm/lootbox labels immediately when it sends those commands, using the configured settings baseline.
...
2. If hunt, adventure, work, farm, or lootbox already has remaining time in that opening snapshot, skip that command and leave its saved setting unchanged.
