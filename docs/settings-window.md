...
- `Bot Parameters` section: area, `Ascended`, a `Work commands` button, a `Guild raid` button, and hunt/adventure/training/work/farm/lootbox cooldown baselines
...
- Existing keys are preserved and extended with `ascended`, `training_ms`, `work_commands`, `guild_raid_channel_url`, `guild_raid_trigger_text`, `guild_raid_match_mode`, and `guild_raid_author_filter`.
